...
uint32_t pci_config(uint8_t bus, uint8_t dev, uint8_t func, uint8_t offset)
{

	uint32_t config_address = 0x800000000 | ((uint32_t)bus << 16) |
				  ((uint32_t)dev << 11) |
				  ((uint32_t)func << 8) |
...
{
	struct pci_id *id =
		(struct pci_id *)pci_config(bus, dev, 0, PCI_VENDOR_ID);
	...
	if ((header_type & PCI_MULTI_FUNCTION) != 0) {
		...
		for (uint8_t f = 1; f < PCI_FUNCTION_NUMBER; f++) {
			id = (struct pci_id *)pci_config(bus, dev, f,
							 PCI_VENDOR_ID);
			if (id->vendor != PCI_NO_DEV) {
...
void pci_enumerate_function(uint8_t bus, uint8_t dev, uint8_t func,
			    struct pci_id id)
{
	struct pci_class *cls = (struct pci_class *)pci_config(
		bus, dev, func, PCI_SUBCLASS_CODE);
	if (cls->cls == PCI_BRIDGE && cls->subclass == PCI_TO_PCI_BRIDGE) {
		// get the secondary bus this pci-to-pci bridge connects to
		uint8_t secondary_bus =
			(uint8_t)pci_config(bus, dev, func, PCI_SECONDARY_BUS);
...
{
	return (id1.vendor == id2.vendor && id1.device == id2.device);
}