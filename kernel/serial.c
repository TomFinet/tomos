...
uint8_t serial_init()
{
	...
	outb(COM1 + SERIAL_LINE_CONTROL,
	     0x03); // 8-bit characters, no parity, 1 stop bit
	outb(COM1 + SERIAL_FIFO,
	     0xC7); // Enable FIFO, clear them, with 14-byte threshold
	outb(COM1 + SERIAL_MODEM_CONTROL, 0x0B); // IRQs enabled, RTS/DSR set
	outb(COM1 + SERIAL_MODEM_CONTROL,
	     0x1E); // Set in loopback mode, test the serial chip
	outb(COM1 + SERIAL_DATA, 0xAE); // Test serial chip (send byte 0xAE and
