...
void idt_init()
{
	struct idt_ptr idtr = {sizeof(struct idt_gate) * IDT_GATE_NUM - 1,
			       (uint32_t)&gates};
	...
	idt_create_descriptor(IDT_IRQ0 + 0, (uint32_t)irq0, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 1, (uint32_t)irq1, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 2, (uint32_t)irq2, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 3, (uint32_t)irq3, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 4, (uint32_t)irq4, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 5, (uint32_t)irq5, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 6, (uint32_t)irq6, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 7, (uint32_t)irq7, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 8, (uint32_t)irq8, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 9, (uint32_t)irq9, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 10, (uint32_t)irq10, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 11, (uint32_t)irq11, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 12, (uint32_t)irq12, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 13, (uint32_t)irq13, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 14, (uint32_t)irq14, 0x08,
			      INTERRUPT_FLAGS);
	idt_create_descriptor(IDT_IRQ0 + 15, (uint32_t)irq15, 0x08,
			      INTERRUPT_FLAGS);
